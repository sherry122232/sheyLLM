...
δt​=rt​+γV(st+1​)−V(st​),A^t​=l=0∑∞​(γλ)lδt+l​.​
...
**核心问题**：把多步未来回报组合成一个低方差的优势估计 A^tA^t​，告诉 actor 这一步比平均好多少。

**核心变量**：

- `delta_t`：TD 误差 δt=rt+γV(st+1)−V(st)δt​=rt​+γV(st+1​)−V(st​)，单步"惊喜"
- `advantage_t`：累积优势 A^tA^t​，从后往前递推
- `gamma`（γγ）：折扣因子，控制目光远近
- `lambda`（λλ）：偏差-方差权衡参数，λ=0λ=0 退化为 TD(0)，λ=1λ=1 退化为 Monte-Carlo
- `done_t`：episode 结束标志，截断跨 episode 的递推

### 一句话记忆

> **从后往前扫：A^t=δt+γλA^t+1A^t​=δt​+γλA^t+1​。**
...
λ=0λ=0 只看单步 TD（方差低、偏差高）；λ=1λ=1 累加所有 δδ（方差高、偏差低）。
...
- `ratio`：rt(θ)=πθ(at∣st)/πθold(at∣st)rt​(θ)=πθ​(at​∣st​)/πθold​​(at​∣st​)，新旧策略概率比（分母是采样时的旧策略）
- `advantage`：A^tA^t​，来自 GAE
- `eps`（ϵϵ）：clip 范围，典型 `0.1` 或 `0.2`
- `new_log_prob` / `old_log_prob`：当前策略与采样时策略的 log 概率

### 一句话记忆

> **比值 rt=πnew/πoldrt​=πnew​/πold​，clip 到 [1−ϵ,1+ϵ][1−ϵ,1+ϵ]；原始与裁剪两个 surrogate 取 min——只信更保守的那个。**

LCLIP=−min⁡(rt(θ)⋅At,  clip(rt(θ),1−ϵ,1+ϵ)⋅At)LCLIP=−min(rt​(θ)⋅At​,clip(rt​(θ),1−ϵ,1+ϵ)⋅At​)
...
**核心问题**：训练 critic 让它准确预测累积回报 V(s)V(s)，并防止它在单次更新中漂移过大（value clipping）。

**核心变量**：

- `value_pred`：critic 当前对 V(st)V(st​) 的预测
- `old_values`：采样时 critic 的预测，用作 clip 参考
- `returns`：Rt=A^t+Vold(st)Rt​=A^t​+Vold​(st​)，GAE 给出的回归目标
- `eps`：clip 范围，与 policy loss 共享

### 一句话记忆

> **(Vpred−R)2(Vpred​−R)2 求均值；可选 clip：预测别离旧预测超过 ϵϵ。**
...
| ratio 分母用错        | 分母必须是采样时的旧策略 πoldπold​，不是当前策略                                        |
| clip 范围写错         | 是围绕 1 的 [1−ϵ,1+ϵ][1−ϵ,1+ϵ]，不是围绕 0 的 [−ϵ,ϵ][−ϵ,ϵ]                     |
